...
PermType                    = 'withinsplit'; % this is invalid, we do it to show what happens to the covariance matrix
...
for iEffectsize         = 1:numel(effectrange)
    Params                      = initializeparams('differenceMagnitude',effectrange(iEffectsize),...
        ...
        'nSamplesPerClass1',40,'nSamplesPerClass2',40,'nDatasets',nDatasets,'nPerm',1e5,...
    ...
    for iDataset                = 1:Params.nDatasets
        ...
        for iFold           = 1:numel(Folds)
            ...
            switch lower(algname)
                case 'svm'
                    [err,errPerm,~,ePall] ...
                                        = classifywithpermutationsmultirunLIBSVM_detailederrors(x,l,Splits,Params1);
                case 'gnb'
...
nameSave    = sprintf('Results%sH1_Detailed_WrongPermutationScheme_%dsamples_%druns_RunVariance%.1f_%drepCV_%diterations_1e5Perm.mat', upper(algname),Params.nSamplesPerClass1+Params.nSamplesPerClass2,...
