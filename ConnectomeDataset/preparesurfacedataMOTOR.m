...
fileexp  = '_tfMRI_MOTOR_level2_hp200_s2_MSMAll.dscalar.nii';
...
ciftinames = dir(fullfile(dirmatdata,'*MOTOR*.nii'));
numcifti   = numel(ciftinames);
contrastmapname = @(s1,s2) ['x' s1 '_tfmri_motor_level2_' s2 '_hp200_s2_msmall'];
...
for idSub    = 1:numcifti
    ...
    labels((idSub-1)*numel(EVNames)+[1:5],1) = [1:5];
    labels((idSub-1)*numel(EVNames)+[1:5],2) = idSub;
    
end
fprintf('\n');        

save('dataConnectomeMotor.mat','features','labels','s0','-v7.3');
% note: this file is large (> 1GB), but it can be recreated from the 889 subjects 
% of the human connectome database (see SubjectIDMOTOR.mat)        
